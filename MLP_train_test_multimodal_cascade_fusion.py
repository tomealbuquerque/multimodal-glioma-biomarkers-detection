...
parser = argparse.ArgumentParser(description='Multimodal MRI + Path - MLP aggregator - cascade with regularization')
...
parser.add_argument('--mix', default='expected',choices=['mix','global', 'expected'], help='get top classes probabilities (3 classes)')
...
parser.add_argument('--model_MRI', type=str, default=r'MRI_weights\multiclass_t1ce\multiclass_fold4_t1ce\multiclass_checkpoint_best_tiles.pth', help='path to trained model checkpoint')
...
def main():
    ...
    # mlp_dict = torch.load(os.path.join(args.results_folder,f'rnn_checkpoint_best_tiles_{args.s}_mix_{args.mix}_loss_{args.weights}_fold_{args.fold}_multimodal.pth'))
    ...
    for epoch in range(args.nepochs):
        ...
        train_loss = train_single(epoch, embedder_PATH,embedder_MRI, mlp, train_loader, criterion, optimizer)
        val_loss, avg_acc,Phat, Phat_p = test_single(epoch, embedder_PATH,embedder_MRI, mlp, val_loader, criterion)
        ...
        if val_err > best_loss:
            ...
            for name, target, prob, pred in zip(val_dset.slidenames, val_dset.targets, Phat_p,Phat):
                
                fp.write('{},{},{},{},{},{}\n'.format(name, target, pred, prob[0],prob[1],prob[2]))
                
            fp.close()
def train_single(epoch, embedder_PATH, embedder_MRI, mlp, loader, criterion, optimizer):
    mlp.train()
    running_loss = 0.
    avg_acc = 0
    avg_auc=0
    ...
    Y_true=[]
    out_mri_all=[]
    for i,(inputs_PATH,inputs_MRI,inputs_age, inputs_gender, target) in enumerate(loader):
        ...
        for s in range(len(inputs_PATH)):
            input = inputs_PATH[s].cuda()
            _, input = embedder_PATH(input)
            inputs.append(input)
            
        # input_mri = embedder_MRI(torch.unsqueeze(inputs_MRI[0], 1).cuda())
        ...
        input = torch.cat((input_mri, torch.cat(inputs, dim=1)), dim=1)
        ...
        # print(Phat_p)
        # avg_auc += roc_auc_score(GT,Phat_p, multi_class='ovr')/len(loader)
        Phat.append(Pred)
        Y_true.append(GT)
        
        if args.MRI_n_outputs in ([3]):
            out_mri=F.softmax(classes_mri, 1).argmax(1)*1
        else:
            out_mri=F.softmax(classes_mri, 1).argmax(1)*2
        out_mri = out_mri.detach().cpu().numpy()
        
        out_mri_all.append(out_mri)
    
    Phat_mri = np.concatenate(out_mri_all)
...
def test_single(epoch,embedder_PATH, embedder_MRI, mlp, loader, criterion):
    mlp.eval()
    running_loss = 0.
    avg_acc = 0
    avg_auc=0
    ...
    with torch.no_grad():
        for i,(inputs_PATH,inputs_MRI,inputs_age, inputs_gender, target)  in enumerate(loader):
            ...
            input_mri, classes_mri = embedder_MRI(inputs_MRI.cuda())
            # input_mri = embedder_MRI(torch.unsqueeze(inputs_MRI[0], 1).cuda())
            # print(F.softmax(input_mri, 1).argmax(1)*1)
            # input = torch.cat((input_mri, torch.cat(inputs, dim=1),torch.unsqueeze(inputs_age.cuda(), 1).float(),torch.unsqueeze(inputs_gender.cuda(), 1).float()), dim=1)
            input = torch.cat((input_mri, torch.cat(inputs, dim=1)), dim=1)
            # input = torch.cat(inputs, dim=1)
            output = mlp(input)
    ...
    return running_loss,avg_acc,Phat,Phat_p
...
class MRI_DensenetEncoder(nn.Module):
    ...
    def __init__(self, path):
        ...
        # temp = nn.Sequential(*tuple(temp.children())[:-1])
        # ch = torch.load(args.model_MRI)
...
class rnndata(data.Dataset):
    ...
    def __getitem__(self,index):
        ...
        return patho, mri, self.age[index], self.gender[index], self.targets[index]
