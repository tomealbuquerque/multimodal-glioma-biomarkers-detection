...
parser = argparse.ArgumentParser(description='Multimodal MRI + Path - MLP aggregator - late fusion')
...
parser.add_argument('--model_PATH', type=str, default='checkpoint_best_512_bin_fold_0.pth', help='path to trained model checkpoint')
parser.add_argument('--model_MRI', type=str, default='MRI_weights\multiclass_t1ce_flair\multiclass_fold0_t1ce_flair\multiclass_checkpoint_best_tiles.pth', help='path to trained model checkpoint')
...
parser.add_argument('--results_folder',default='testes')
...
def main():
    ...
    scheduler = optim.lr_scheduler.ReduceLROnPlateau(optimizer, verbose=True,patience=4)
    ...
    for epoch in range(args.nepochs):
        ...
        train_loss = train_single(epoch, embedder_PATH,embedder_MRI, mlp, train_loader, criterion, optimizer,scheduler)
        val_loss, avg_acc, Phat, probs = test_single(epoch, embedder_PATH,embedder_MRI, mlp, val_loader, criterion)
...
def train_single(epoch, embedder_PATH, embedder_MRI, mlp, loader, criterion, optimizer,scheduler):
    mlp.train()
    running_loss = 0.
    avg_acc = 0
    avg_auc=0
    ...
    for i,(inputs_PATH,inputs_MRI,inputs_age, inputs_gender, target) in enumerate(loader):
        ...
        output = mlp(torch.cat(inputs, dim=1),classes_mri)
...
class MLP_single(nn.Module):
    ...
    def __init__(self, ndims):
        ...
        self.model_path = nn.Sequential(
            nn.Linear(args.s*512, 512),
            nn.Dropout(0.1),
            nn.ReLU(),
            nn.Linear(512, 256),
            nn.ReLU(),
        ...
        self.model_fusion = nn.Sequential(
            nn.Linear(3+args.MRI_n_outputs, 12),
            nn.ReLU(),
            nn.Linear(12, 3)
        )


    def forward(self, input, input_MRI):
        output = self.model_path(input)
        output = self.model_fusion(torch.cat((output,input_MRI), dim=1))
        return output
