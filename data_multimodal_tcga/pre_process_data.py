...
# create a new dataframe with all the multimodal data available
...
for idx, pat in enumerate(df_clean['subject_id']):
    ...
    """
    X_full_data.append({'flair': image_modality[0],
                        't1': image_modality[1],
                        't1ce': image_modality[2],
                        't2': image_modality[3],
    ...
    """
    Y_full_data.append(
        {'idh1': int(df_clean.loc[idx, 'IDH1_mut']), 'ioh1p15q': int(df_clean.loc[idx, 'loh1p/19q_cnv'])})
