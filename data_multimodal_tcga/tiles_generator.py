...
def get_tiles(slide_path, tile_size=512, threshold_for_otsu=0.4):
    ...
    slide_thumb = slide.get_thumbnail(
        (int(np.floor(slide_dims[0] / down_factor)), int(np.floor(slide_dims[1] / down_factor))))
    ...
    while y < y_tiles:
        while x < x_tiles:
            with np.errstate(divide='ignore'):
                ...
                if (np.sum(new_tile_otsu, axis=2) == 0).sum() <= np.round(
                        (1 - threshold) * (tile_size / down_factor) ** 2):
